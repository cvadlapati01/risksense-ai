...
// ============= AI-generated risks store (in-memory, client-side) =============
let aiRisks: Risk[] = [];
const aiRiskSubs = new Set<() => void>();

export function getAiRisks(): Risk[] {
  return aiRisks;
}

export function subscribeAiRisks(cb: () => void): () => void {
  aiRiskSubs.add(cb);
  return () => aiRiskSubs.delete(cb);
}

export function addAiRisks(
  items: Array<{
    title: string;
    description: string;
    category: RiskCategory;
  ...
  const base = aiRisks.length;
  const created = items.map((m, i) => {
    const id = `AI-${(base + i + 1).toString().padStart(3, "0")}`;
    return makeRisk({
      id,
      title: m.title,
      ...
      owner: "AI Suggestion",
      dueDate: "TBD",
      mitigation: m.mitigations.join(" "),
      aiRecommendations: m.mitigations,
      actionsCompleted: 0,
      actionsTotal: m.mitigations.length,
      residual: "Moderate",
      history: [{ date: "TODAY", note: "Generated by AI Risk Analyzer" }],
    });
  });
  aiRisks = [...created, ...aiRisks];
  aiRiskSubs.forEach((cb) => cb());
  return created;
}
